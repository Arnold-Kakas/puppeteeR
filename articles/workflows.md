...
settled. The routing checks whether the judge’s response contains the
word `"done"` (case-insensitive) anywhere — if found, the workflow ends;
otherwise it continues. The judge can write a full response as long as
it ends with `done` or `continue`.
...
**Custom `state_schema` and the `output` channel**: when you supply a
`state_schema`, only the channels you declare exist. The `output`
channel is optional — the workflow runs fine without it, but
`result$output()` will error because there is no nominated output
channel. Add `output = list(default = "")` to your schema to re-enable
it:
...
**`max_rounds` with a judge**: each round consumes 3 node executions
(optimist + pessimist + judge), but the internal termination guard is
`max_turns(max_rounds * 2)` — it does not account for judge turns. Set
`max_rounds` higher than you need to ensure the judge gets enough
...
| [`advisor_workflow()`](https://arnold-kakas.github.io/puppeteeR/reference/advisor_workflow.md)       | Quality gate: cheap model generates, expensive model approves     |
| [`planner_workflow()`](https://arnold-kakas.github.io/puppeteeR/reference/planner_workflow.md)       | Cost efficiency: expensive model plans once, cheap models execute |
...
For guidance on model selection, context management, retry
configuration, and prompt design see
