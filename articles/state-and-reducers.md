...
`reducer_last_n()` is particularly useful for long-running supervisor or
debate workflows where you want to prevent the context window from
growing too large.
...
state_schema <- workflow_state(
  messages            = list(default = list(), reducer = reducer_append()),
  plan                = list(default = list()),          # structured plan steps
