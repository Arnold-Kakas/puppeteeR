...
3. Calls `state$restore(snapshot)` — this writes the saved values directly into the state, bypassing reducers.
4. Sets `start_step` to the last saved step number, so subsequent saves don't overwrite history.
5. **Starts executing from the entry node (first node in the graph)**. There is no automatic node-skipping; execution always begins at the start.

The practical implication: **resumed nodes re-execute, but they find the state already populated**. Nodes designed to overwrite their output channel will replace it with a (likely identical) new value. Nodes that accumulate into an `append` reducer will add to the already-restored list. See the [Idempotent node patterns](#idempotent-node-patterns) section for how to handle this.
...
```{r idempotent-status, eval = FALSE}
...
1. Loads the latest checkpoint for the thread.
2. Applies the updates to the snapshot (no reducers — direct replacement).
3. Saves a new checkpoint at `last_step + 1`.
